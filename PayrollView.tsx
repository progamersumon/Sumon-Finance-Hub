      ...
      {activePayrollSubView === 'Pay Slip' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card title="Monthly Breakdown" className="h-auto sm:h-[360px] dark:bg-slate-900 dark:border-slate-800">
              <div className="space-y-0.5 h-full overflow-y-auto no-scrollbar pr-1 pb-4">
                {[{ label: 'BASIC SALARY', value: basicSalary }, { label: 'HOUSE RENT ALLOWANCE', value: houseRent }, { label: 'MEDICAL ALLOWANCE', value: salaryConfig.medical || 0 }, { label: 'CONVEYANCE ALLOWANCE', value: salaryConfig.conveyance || 0 }, { label: 'FOOD ALLOWANCE', value: salaryConfig.food || 0 }, { label: 'ATTENDANCE BONUS', value: salaryConfig.attendanceBonus || 0 }, { label: `TIFFIN BILL (${salaryConfig.tiffinDays || 0} DAYS)`, value: tiffinBillAmount }].map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center py-1.5 px-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-all border-b border-slate-50/50 dark:border-slate-800/30 last:border-0">
                    <div className="flex items-center gap-2.5">
                       <div className="w-1.5 h-1.5 rounded-full bg-blue-500/40"></div>
                       <span className="text-[9px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tight">{item.label}</span>
                    </div>
                    <span className="text-[11px] sm:text-sm font-black text-slate-800 dark:text-white bg-slate-100/30 dark:bg-slate-800 px-2 py-0.5 rounded-md">৳{(item.value || 0).toLocaleString()}</span>
                  </div>
                ))}
                <div className="mt-2.5 p-3 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-700 dark:from-blue-600/90 dark:to-indigo-700/90 shadow-md flex justify-between items-center border border-white/5">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-black text-blue-100 uppercase tracking-[0.15em]">Summary</span>
                    <span className="text-[10px] sm:text-xs font-black text-white uppercase tracking-wider">Total Monthly</span>
                  </div>
                  <span className="text-lg sm:text-xl font-black text-white">৳{totalMonthly.toLocaleString()}</span>
                </div>
              </div>
            </Card>

            <Card title="Bonuses & Festivals" className="h-auto sm:h-[360px] dark:bg-slate-900 dark:border-slate-800">
              <div className="space-y-0.5 h-full overflow-y-auto no-scrollbar pr-1 pb-4">
                ...
                <div className="mt-2.5 p-3 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-700 dark:from-emerald-600/90 dark:to-teal-800/90 shadow-md flex justify-between items-center border border-white/5">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-black text-emerald-100 uppercase tracking-[0.15em]">Payable</span>
                    <span className="text-[10px] sm:text-xs font-black text-white uppercase tracking-wider">Total Yearly</span>
                  </div>
                  <span className="text-lg sm:text-xl font-black text-white">৳{totalYearlyBonus.toLocaleString()}</span>
                </div>
              </div>
            </Card>
          </div>
