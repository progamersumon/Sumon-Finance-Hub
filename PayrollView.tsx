...
export const PayrollView: React.FC<PayrollViewProps> = ({ 
  ...
  const basicSalary = Math.round((Number(salaryConfig?.grossSalary || 0) - DEFAULT_BASE_DEDUCTION) / 1.5);
  const houseRent = Math.round(basicSalary / 2);
  const tiffinBillAmount = (Number(salaryConfig?.tiffinDays || 0)) * 50;
  
  const totalMonthly = useMemo(() => {
    const sum = 
      (basicSalary || 0) + 
      (houseRent || 0) + 
      (Number(salaryConfig?.medical || 0)) + 
      (Number(salaryConfig?.conveyance || 0)) + 
      (Number(salaryConfig?.food || 0)) + 
      (Number(salaryConfig?.attendanceBonus || 0)) + 
      (tiffinBillAmount || 0);
    return sum;
  }, [basicSalary, houseRent, salaryConfig, tiffinBillAmount]);

  const yearlyBonus = Math.round(Number(salaryConfig?.grossSalary || 0) / 1.5);
      ...
      {activePayrollSubView === 'Pay Slip' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
          <div className="bg-white dark:bg-slate-900 px-4 sm:px-6 py-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex justify-between items-center">
             <div className="flex items-center gap-2 sm:gap-3"><p className="text-[10px] sm:text-sm font-bold text-slate-400 uppercase tracking-widest">Gross Salary</p><h3 className="text-lg sm:text-2xl font-black text-slate-800 dark:text-white">৳{(Number(salaryConfig?.grossSalary || 0)).toLocaleString()}</h3></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-4">
            <Card title="Monthly Breakdown" className="flex flex-col h-auto sm:h-[650px] dark:bg-slate-900 dark:border-slate-800 p-0 overflow-hidden shadow-lg border-2 border-blue-100 dark:border-blue-900/20">
              <div className="p-4 sm:p-6 flex-1 overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900/40">
                <div className="space-y-2">
                  {[{ label: 'BASIC SALARY', value: basicSalary }, { label: 'HOUSE RENT ALLOWANCE', value: houseRent }, { label: 'MEDICAL ALLOWANCE', value: Number(salaryConfig?.medical || 0) }, { label: 'CONVEYANCE ALLOWANCE', value: Number(salaryConfig?.conveyance || 0) }, { label: 'FOOD ALLOWANCE', value: Number(salaryConfig?.food || 0) }, { label: 'ATTENDANCE BONUS', value: Number(salaryConfig?.attendanceBonus || 0) }, { label: `TIFFIN BILL (${Number(salaryConfig?.tiffinDays || 0)} DAYS)`, value: tiffinBillAmount }].map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center py-4 border-b border-slate-50 dark:border-slate-800 last:border-0 transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/30 px-2 rounded-lg">
                      <span className="text-[10px] sm:text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">{item.label}</span>
                      <span className="text-sm sm:text-lg font-black text-slate-800 dark:text-white">৳{(Number(item.value) || 0).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="p-5 sm:px-8 py-6 sm:py-8 bg-slate-900 text-white dark:bg-blue-600 dark:text-white border-t-4 border-blue-500 dark:border-blue-400 rounded-b-xl shrink-0">
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm font-black uppercase tracking-[0.2em] opacity-80">Total Monthly</span>
                  <span className="text-xl sm:text-4xl font-black tracking-tighter drop-shadow-md">৳{(totalMonthly || 0).toLocaleString()}</span>
                </div>
              </div>
            </Card>
            <Card title="Bonuses & Festivals" className="flex flex-col h-auto sm:h-[650px] dark:bg-slate-900 dark:border-slate-800 p-0 overflow-hidden shadow-lg border-2 border-emerald-100 dark:border-emerald-900/20">
              <div className="p-4 sm:p-6 flex-1 overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900/40">
                <div className="space-y-2">
              ...
              <div className="p-5 sm:px-8 py-6 sm:py-8 bg-slate-900 text-white dark:bg-emerald-600 dark:text-white border-t-4 border-emerald-500 dark:border-emerald-400 rounded-b-xl shrink-0">
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm font-black uppercase tracking-[0.2em] opacity-80">Payable (Yearly)</span>
                  <span className="text-xl sm:text-4xl font-black tracking-tighter drop-shadow-md">৳{(totalYearlyBonus || 0).toLocaleString()}</span>
                </div>
              </div>
            </Card>
          </div>
