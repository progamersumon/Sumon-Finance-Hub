    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      {activePayrollSubView === 'Salary' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
          ...
          <button onClick={() => { setEditingHistoryId(null); const sortedForNext = [...historyEntries].sort((a, b) => a.year.localeCompare(a.year)); const last = sortedForNext[sortedForNext.length - 1]; setPrevTotalForCalc(last ? last.total : 0); setHistoryFormData({ year: (parseInt(last?.year || '2025') + 1).toString(), baseDeduction: lastUsedBaseDeduction.toString(), increasePercent: '', amountAdd: '', total: last ? last.total.toString() : '' }); setIsHistoryModalOpen(true); }} className="fixed bottom-24 right-6 lg:bottom-12 lg:right-12 w-14 h-14 sm:w-16 sm:h-16 bg-slate-800 dark:bg-slate-700 text-white rounded-full shadow-2xl flex items-center justify-center text-3xl sm:text-4xl font-light hover:scale-110 active:scale-95 transition-all z-[70] group"><span className="group-hover:rotate-90 transition-transform duration-300">+</span></button>
        </div>
      )}
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
          <div className="bg-white dark:bg-slate-900 px-4 sm:px-6 py-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex justify-between items-center">
             <div className="flex items-center gap-2 sm:gap-3">
               ...
               <div>
                 <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">Gross Salary</p>
                 <h3 className="text-lg sm:text-2xl font-black text-slate-800 dark:text-white leading-none">৳{salaryConfig.grossSalary.toLocaleString()}</h3>
               </div>
             </div>
             <button onClick={() => { setEditFormData(salaryConfig); setIsEditModalOpen(true); }} className="flex items-center space-x-1 px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all shadow-md active:scale-95">
               <ICONS.Edit className="w-3.5 h-3.5" />
               <span className="text-[10px] sm:text-xs font-bold uppercase">Configure</span>
             </button>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Monthly Breakdown Card */}
            <Card className="h-auto dark:bg-slate-900 dark:border-slate-800 overflow-hidden !p-0">
              <div className="px-6 py-4 border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
              ...
              <div className="p-4 sm:p-5 space-y-0.5">
                {[
                  ...
                  { label: 'Medical', value: salaryConfig.medical || 0, icon: <HeartPulse size={14} />, color: 'rose' },
                  { label: 'Conveyance', value: salaryConfig.conveyance || 0, icon: <Car size={14} />, color: 'indigo' },
                  { label: 'Food', value: salaryConfig.food || 0, icon: <Utensils size={14} />, color: 'orange' },
                  ...
                  <div key={idx} className="group flex items-center justify-between p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-all">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[11px] sm:text-xs font-bold text-slate-700 dark:text-slate-300">{item.label}</p>
                        {item.subtitle && <p className="text-[9px] font-medium text-slate-400 dark:text-slate-500 uppercase">{item.subtitle}</p>}
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xs sm:text-sm font-black text-slate-800 dark:text-white">৳{(item.value || 0).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
                ...
                {[
                  ...
                  <div key={idx} className="group flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-all border border-transparent hover:border-slate-100 dark:hover:border-slate-800">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-xs sm:text-sm font-black text-slate-800 dark:text-white">{item.label}</p>
                        <p className="text-[10px] font-medium text-slate-400 dark:text-slate-500 italic">{item.desc}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className={`text-sm sm:text-lg font-black text-${item.color}-600 dark:text-${item.color}-400`}>৳{(item.value || 0).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Configuration">
        <div className="space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar pr-1">
          ...
          <button onClick={handleEditSave} className="w-full py-3 sm:py-4 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-all active:scale-95 mt-4">Update Configuration</button>
        </div>
      </Modal>
    </div>
  );
