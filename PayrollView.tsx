        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card title="Monthly Breakdown" className="h-auto sm:h-[420px] dark:bg-slate-900 dark:border-slate-800">
              <div className="space-y-1 h-full overflow-y-auto custom-scrollbar pr-1 pb-4">
                {[{ label: 'BASIC SALARY', value: basicSalary }, { label: 'HOUSE RENT ALLOWANCE', value: houseRent }, { label: 'MEDICAL ALLOWANCE', value: salaryConfig.medical || 0 }, { label: 'CONVEYANCE ALLOWANCE', value: salaryConfig.conveyance || 0 }, { label: 'FOOD ALLOWANCE', value: salaryConfig.food || 0 }, { label: 'ATTENDANCE BONUS', value: salaryConfig.attendanceBonus || 0 }, { label: `TIFFIN BILL (${salaryConfig.tiffinDays || 0} DAYS)`, value: tiffinBillAmount }].map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center py-2 px-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                    ...
                    <span className="text-xs sm:text-sm font-black text-slate-800 dark:text-white bg-slate-100/50 dark:bg-slate-800 px-3 py-1 rounded-lg">৳{(item.value || 0).toLocaleString()}</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 p-4 rounded-[20px] bg-gradient-to-br from-blue-600 to-blue-700 dark:from-blue-600/90 dark:to-indigo-700/90 shadow-lg shadow-blue-200 dark:shadow-none flex justify-between items-center border border-white/10">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black text-blue-100 uppercase tracking-[0.2em] mb-0.5">Summary</span>
                  <span className="text-xs sm:text-sm font-black text-white uppercase tracking-wider">Total Monthly</span>
                </div>
                <span className="text-xl sm:text-2xl font-black text-white drop-shadow-sm">৳{totalMonthly.toLocaleString()}</span>
              </div>
            </Card>
            ...
            <Card title="Bonuses & Festivals" className="h-auto sm:h-[420px] dark:bg-slate-900 dark:border-slate-800">
              ...
              <div className="mt-4 p-4 rounded-[20px] bg-gradient-to-br from-emerald-600 to-teal-700 dark:from-emerald-600/90 dark:to-teal-800/90 shadow-lg shadow-emerald-200 dark:shadow-none flex justify-between items-center border border-white/10">
                <div className="flex flex-col">
                  <span className="text-[10px] font-black text-emerald-100 uppercase tracking-[0.2em] mb-0.5">Payable</span>
                  <span className="text-xs sm:text-sm font-black text-white uppercase tracking-wider">Total Yearly</span>
                </div>
                <span className="text-xl sm:text-2xl font-black text-white drop-shadow-sm">৳{totalYearlyBonus.toLocaleString()}</span>
              </div>
            </Card>
          </div>
        </div>
