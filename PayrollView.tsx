      ...
      {activePayrollSubView === 'Pay Slip' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-4">
            <Card title="Monthly Breakdown" className="flex flex-col h-auto sm:h-[520px] dark:bg-slate-900 dark:border-slate-800 p-0 overflow-hidden shadow-lg">
              <div className="p-4 sm:p-6 flex-1 overflow-y-auto custom-scrollbar">
                <div className="space-y-2">
                  {[{ label: 'BASIC SALARY', value: basicSalary }, { label: 'HOUSE RENT ALLOWANCE', value: houseRent }, { label: 'MEDICAL ALLOWANCE', value: Number(salaryConfig?.medical || 0) }, { label: 'CONVEYANCE ALLOWANCE', value: Number(salaryConfig?.conveyance || 0) }, { label: 'FOOD ALLOWANCE', value: Number(salaryConfig?.food || 0) }, { label: 'ATTENDANCE BONUS', value: Number(salaryConfig?.attendanceBonus || 0) }, { label: `TIFFIN BILL (${Number(salaryConfig?.tiffinDays || 0)} DAYS)`, value: tiffinBillAmount }].map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center py-2.5 border-b border-slate-50 dark:border-slate-800 last:border-0 transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/30 px-1 rounded-lg">
                      ...
                      <span className="text-xs sm:text-sm font-black text-slate-800 dark:text-white">৳{(Number(item.value) || 0).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="p-5 sm:px-6 pb-8 sm:pb-12 bg-slate-50 dark:bg-slate-800/50 border-t-2 border-slate-100 dark:border-slate-800 rounded-b-xl">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg sm:text-3xl font-black text-blue-700 dark:text-white">৳{(totalMonthly || 0).toLocaleString()}</span>
                </div>
              </div>
            </Card>
            <Card title="Bonuses & Festivals" className="flex flex-col h-auto sm:h-[520px] dark:bg-slate-900 dark:border-slate-800 p-0 overflow-hidden shadow-lg">
              <div className="p-4 sm:p-6 flex-1 overflow-y-auto custom-scrollbar">
                <div className="space-y-2">
              ...
              <div className="p-5 sm:px-6 pb-8 sm:pb-12 bg-slate-50 dark:bg-slate-800/50 border-t-2 border-slate-100 dark:border-slate-800 rounded-b-xl">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg sm:text-3xl font-black text-emerald-700 dark:text-white">৳{(totalYearlyBonus || 0).toLocaleString()}</span>
                </div>
              </div>
            </Card>
          </div>
